...
for i in input_array:
    itemchosen = [p['name'] for p in products if str(p['id']) == str(i)]
    itemprice = [p['price'] for p in products if str(p['id']) == str(i)]
    item = input_array[loopcount] - 1
    print(" + ", itemchosen[0],"  $" + str(itemprice[0])) 
    loopprice = float(str(itemprice[0]))
